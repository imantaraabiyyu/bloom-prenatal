...
1) category = "food" — user cerita atau menunjukkan foto tentang makanan/minuman yang sedang/baru dia
   ...
   - isi field gizi "target minimum" berikut (nama field, satuan, nama gizinya):
     ${NUTRIENT_LIST_FOR_PROMPT}. Isi juga field gizi "batas maksimum harian" berikut (jangan sampai
     terlewat, terutama untuk kopi/teh/makanan kemasan/gorengan/makanan asin/manis):
     ${LIMIT_LIST_FOR_PROMPT}. Kalau fotonya adalah label kemasan dengan angka gizi tercetak, BACA
   ...
   - extra_nutrients = daftar nutrisi/komposisi tambahan di luar daftar di atas kalau ada info
     gizinya (mis. dari label kemasan yang mencantumkan Omega-3, Zinc, Vitamin B6, dll — masing-
     masing {label, unit, value}), array kosong kalau tidak ada nutrisi tambahan yang terlihat.
...
2) category = "vitamin" — foto atau cerita tentang vitamin/suplemen: label botol vitamin/suplemen,
   ...
   - vitamins = array berisi SATU OBJEK PER vitamin/suplemen yang terdeteksi (boleh lebih dari satu
     ...
     - isi field gizi berikut untuk produk itu (${NUTRIENT_LIST_FOR_PROMPT}, ${LIMIT_LIST_FOR_PROMPT})
...
asupan gula/natrium/kolesterol/lemak jenuh/kafein yang SUDAH tercatat hari ini SEBELUM menu ini,
disisipkan otomatis oleh sistem. Pakai itu untuk menilai apakah menambah menu ini membuat totalnya
...
// further API keys to try if the primary one is exhausted/invalid. Not
// tier-specific (unlike the model chains above): an API key is an
// account-level credential, not something that varies by which feature is
// calling, so this one chain is shared by every call site.
...
// VERDICT section refers to — one clause per LIMIT_ORDER nutrient already
// consumed today (already-saved meals + checked vitamins only, see
// app/api/nutrition-chat/route.js for where dailyLimitTotals comes from) —
// so Gemini's verdict can reason about today's cumulative intake, not just
// this one item in isolation. Returns null when there's nothing to add (no
// `dailyLimitTotals` passed, e.g. a caller that hasn't wired it up, or every
// value is exactly 0) so callers never prepend an empty/pointless line.
function buildDailyContextHint(dailyLimitTotals) {
  if (!dailyLimitTotals) return null;
  const hasAny = LIMIT_ORDER.some((k) => (Number(dailyLimitTotals[k]) || 0) > 0);
  if (!hasAny) return null;
  const parts = LIMIT_ORDER.map((k) => {
    const used = Math.round((Number(dailyLimitTotals[k]) || 0) * 10) / 10;
    return `${LIMIT_META[k].label} ${used}${LIMIT_META[k].unit} dari batas ${LIMITS[k]}${LIMIT_META[k].unit}`;
  });
...
// the 5 LIMIT_ORDER nutrients (see app/api/nutrition-chat/route.js, which
// fetches it server-side before calling this) — folded into the prompt via
// buildDailyContextHint so the food-category verdict can reason about
// today's intake so far, not just this one item.
export async function streamNutritionChatTurn({ message, image, history, dailyLimitTotals }, onDelta) {
  ...
  const { res, model, usedLowerTier } = await fetchGeminiForChat(
    (model, apiKey) => `https://generativelanguage.googleapis.com/v1beta/models/${model}:streamGenerateContent?alt=sse&key=${apiKey}`,
    {
      ...
      contents: buildContents({ message, image, history, contextHint: buildDailyContextHint(dailyLimitTotals) }),
