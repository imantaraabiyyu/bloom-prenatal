...
async function fetchGeminiWithRetries(url, requestBody, { requireBody = false } = {}) {
...
// Used only by app/api/journal/transcribe/route.js. Not streamed — unlike
// the chat, there's nothing useful to show mid-transcript, the point is
// reviewing the finished tidied text once, not watching it type out.
...
export async function transcribeVoiceNote({ base64, mimeType }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error("GEMINI_API_KEY belum diisi di .env.local / Vercel (lihat README).");
  const model = process.env.GEMINI_MODEL || "gemini-3.6-flash";
