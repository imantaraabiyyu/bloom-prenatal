...
// model on ANY GeminiHttpError from the current one — not just 429 (quota),
// also e.g. 404 (Google retired/renamed the model — happened in practice:
// gemini-2.5-pro started returning "no longer available to new users" one
// day with no code change here) or 400/403. A raw non-HTTP exception (network
// failure — fetchGeminiWithRetries never wraps those in GeminiHttpError, see
// its own comment) still isn't caught here, so it still propagates
// immediately without cycling the whole chain for something no model in it
// could fix anyway. Returns { res, model } for whichever model served it.
//
...
// now that exhausted failure also advances to the next model, so a long
// GEMINI_FALLBACK_MODELS chain multiplies that cost per user. Accepted since
// the cron route already degrades per-user on any failure regardless of how
// long it took to get there (see generateDailyNudge/generateMealFact below).
async function tryModelChain(models, buildUrl, requestBody, opts) {
  for (let i = 0; i < models.length; i++) {
    ...
    try {
      ...
      if (!(e instanceof GeminiHttpError) || isLastModel) throw e;
      console.warn(`Gemini model "${model}" failed (status ${e.status}), falling back to "${models[i + 1]}"`);
    }
  }
}
