...
import {
  ...
  buildLimitWarningClause, WEIGHT_NUDGE_TEXT,
...
describe("buildReminderBody", () => {
  ...
  // New case the dinner-slot trigger change unlocks: a user who logged
  // everything (both false) but is over a daily limit still gets a body
  // worth sending, via the new limitWarning param.
  it("appends the limit warning after the nudge, even when nothing is missing", () => {
    const body = buildReminderBody({
      missingMeal: false, missingVitamin: false, nudge: "Semangat!",
      limitWarning: "⚠️ Sudah melebihi batas harian Natrium hari ini.",
    });
    expect(body).toBe("Semua sudah tercatat hari ini! Semangat! ⚠️ Sudah melebihi batas harian Natrium hari ini.");
  });

  it("appends the limit warning alongside a missing-meal/vitamin lead too", () => {
    const body = buildReminderBody({
      missingMeal: true, missingVitamin: false, nudge: "Semangat!",
      limitWarning: "⚠️ Sudah melebihi batas harian Gula hari ini.",
    });
    expect(body).toBe("Belum catat menu hari ini. Semangat! ⚠️ Sudah melebihi batas harian Gula hari ini.");
  });

  it("omits the limit warning entirely when it's an empty string", () => {
    const body = buildReminderBody({ missingMeal: true, missingVitamin: false, nudge: "Semangat!", limitWarning: "" });
    expect(body).toBe("Belum catat menu hari ini. Semangat!");
  });
});

// Shared source of truth with the Dashboard's own warning banner
// (app/dashboard/page.js) — both driven by lib/nutrition.js's
// exceededLimitLabels.
describe("buildLimitWarningClause", () => {
  it("returns an empty string for no exceeded nutrients", () => {
    expect(buildLimitWarningClause([])).toBe("");
    expect(buildLimitWarningClause(undefined)).toBe("");
    expect(buildLimitWarningClause(null)).toBe("");
  });

  it("names a single exceeded nutrient", () => {
    expect(buildLimitWarningClause(["Natrium"])).toBe(
      "⚠️ Sudah melebihi batas harian Natrium hari ini — coba dikurangi dulu ya."
    );
  });

  it("joins multiple exceeded nutrients with a comma", () => {
    expect(buildLimitWarningClause(["Gula", "Natrium"])).toBe(
      "⚠️ Sudah melebihi batas harian Gula, Natrium hari ini — coba dikurangi dulu ya."
    );
  });
});
...
describe("buildMorningBody / buildLunchBody / buildNightBody", () => {
  ...
  // Regression guard: an existing caller that doesn't pass weightNudge at all
  // (the shape every call site used before this feature) must behave exactly
  // as before.
  it("morning omits the weight nudge entirely when not passed", () => {
    expect(buildMorningBody({ name: "Sarah", nudge: "Semangat hari ini!" })).toBe("Pagi, Sarah! Semangat hari ini!");
  });

  it("morning appends the weight nudge after the regular nudge when passed", () => {
    const body = buildMorningBody({ name: "Sarah", nudge: "Semangat hari ini!", weightNudge: WEIGHT_NUDGE_TEXT });
    expect(body).toBe(`Pagi, Sarah! Semangat hari ini! ${WEIGHT_NUDGE_TEXT}`);
  });

  it("morning omits the weight nudge when it's an empty string", () => {
    expect(buildMorningBody({ name: "Sarah", nudge: "Semangat hari ini!", weightNudge: "" }))
      .toBe("Pagi, Sarah! Semangat hari ini!");
  });
...
describe("sumReminderResults", () => {
  it("sums sent/pruned/skipped/emailed across multiple user results", () => {
    const results = [
      { sent: 2, pruned: 0, skipped: 0, emailed: 0 }, // user with 2 devices, both sent
      { sent: 0, pruned: 1, skipped: 0, emailed: 0 }, // user with 1 dead subscription, pruned
      { sent: 0, pruned: 0, skipped: 1, emailed: 0 }, // dinner slot: already logged, skipped
    ];
    expect(sumReminderResults(results)).toEqual({ sent: 2, pruned: 1, skipped: 1, emailed: 0 });
  });

  it("returns all zeros for an empty result list (e.g. no subscribers)", () => {
    expect(sumReminderResults([])).toEqual({ sent: 0, pruned: 0, skipped: 0, emailed: 0 });
  });

  it("handles a mix of successes, prunes, and skips in the same batch", () => {
    const results = [
      { sent: 1, pruned: 1, skipped: 0, emailed: 0 }, // one device sent, one pruned, same user
      { sent: 3, pruned: 0, skipped: 0, emailed: 0 },
      { sent: 0, pruned: 0, skipped: 1, emailed: 0 },
      { sent: 0, pruned: 0, skipped: 1, emailed: 0 },
    ];
    expect(sumReminderResults(results)).toEqual({ sent: 4, pruned: 1, skipped: 2, emailed: 0 });
  });

  it("sums emailed alongside the others when a user's push fully failed and the email fallback caught it", () => {
    const results = [
      { sent: 0, pruned: 1, skipped: 0, emailed: 1 }, // dead subscription, pruned, fallback email sent
      { sent: 2, pruned: 0, skipped: 0, emailed: 0 }, // another user, push worked fine
      { sent: 0, pruned: 0, skipped: 0, emailed: 0 }, // transient push failure, email fallback also failed/unset
    ];
    expect(sumReminderResults(results)).toEqual({ sent: 2, pruned: 1, skipped: 0, emailed: 1 });
  });

  it("doesn't throw on a missing/undefined input", () => {
    expect(() => sumReminderResults(undefined)).not.toThrow();
    expect(sumReminderResults(undefined)).toEqual({ sent: 0, pruned: 0, skipped: 0, emailed: 0 });
  });
});
