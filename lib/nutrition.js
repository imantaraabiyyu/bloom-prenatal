...
// Given a `{ [limitKey]: number }` totals object (same shape dayLimitTotals/
// groupLimitTotalsByUser below produce), returns the display label of every
// LIMIT_ORDER nutrient strictly over its LIMITS number — single source of
// truth shared by the dashboard's warning banner (app/dashboard/page.js) and
// the dinner-reminder trigger/message (lib/reminderLogic.js,
// app/api/cron/reminders/route.js) so "what counts as exceeded" can't drift
// between the two.
export function exceededLimitLabels(totals) {
  return LIMIT_ORDER.filter((k) => (totals?.[k] || 0) > LIMITS[k]).map((k) => LIMIT_META[k].label);
}
...
// Sums LIMIT_ORDER fields across arbitrary rows (already filtered to one
// target date), grouped by user_id -- multi-user variant used server-side by
...
export function groupLimitTotalsByUser(rows) {
  const byUser = {};
  (rows || []).forEach((r) => {
    const uid = r.user_id;
    if (!uid) return;
    if (!byUser[uid]) { byUser[uid] = {}; LIMIT_ORDER.forEach((k) => { byUser[uid][k] = 0; }); }
    LIMIT_ORDER.forEach((k) => { if (r[k] != null) byUser[uid][k] += Number(r[k]) || 0; });
  });
  return byUser;
}

// Adds two `{ [userId]: { [limitKey]: number } }` maps together per-user,
// per-nutrient — e.g. combining groupLimitTotalsByUser's meals-rows result
// with its checked-vitamins-rows result into one running total per user.
export function mergeUserTotals(a, b) {
  const out = {};
  [a, b].forEach((totals) => {
    Object.entries(totals || {}).forEach(([uid, vals]) => {
      if (!out[uid]) { out[uid] = {}; LIMIT_ORDER.forEach((k) => { out[uid][k] = 0; }); }
      LIMIT_ORDER.forEach((k) => { out[uid][k] += vals?.[k] || 0; });
    });
  });
  return out;
}
