...
import { resolveEffectiveGoals } from "@/lib/nutrition";
...
const {
  sanitizeChatTurnResult, isGeminiBusyError, GeminiHttpError, generateDailyNudge, generateMealFact,
...
// generateMealFact is called once per distinct trimester BUCKET by
// app/api/cron/reminders/route.js's lunch slot (not once per user) — these
// assert the request body actually carries the bucket's trimester context
// and its 7-day avoid-list, the two behavioral changes from the "shared,
// non-repeating lunch fun fact" improvement.
describe("generateMealFact (bucketed lunch fun fact)", () => {
  const factBody = (fact) => ({ candidates: [{ content: { parts: [{ text: JSON.stringify({ fact }) }] } }] });

  it("includes the bucket's trimester context and no avoid-list block when there's no history yet", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_MODEL = "low-model-a";
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => factBody("Folat penting banget di trimester 2.") });
    vi.stubGlobal("fetch", fetchMock);
    try {
      const fact = await generateMealFact({ trimester: "t2", avoidFacts: [] });
      expect(fact).toBe("Folat penting banget di trimester 2.");
      const requestBody = JSON.parse(fetchMock.mock.calls[0][1].body);
      const contextText = requestBody.contents[0].parts[0].text;
      expect(contextText).toContain("trimester 2");
      expect(contextText).not.toContain("SUDAH dipakai"); // no avoid-list block when avoidFacts is empty
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_MODEL;
    }
  ...
  it("falls back to the generic 'trimester unknown' context for the unknown bucket", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_MODEL = "low-model-a";
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => factBody("Cairan yang cukup penting selama hamil.") });
    vi.stubGlobal("fetch", fetchMock);
    try {
      await generateMealFact({ trimester: null, avoidFacts: [] });
      const requestBody = JSON.parse(fetchMock.mock.calls[0][1].body);
      expect(requestBody.contents[0].parts[0].text).toContain("belum diketahui");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_MODEL;
    }
  ...
  it("includes every avoid-listed fact and a don't-repeat instruction when the bucket has recent history", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_MODEL = "low-model-a";
    const fetchMock = vi.fn().mockResolvedValue({ ok: true, json: async () => factBody("Zat besi juga penting trimester ini.") });
    vi.stubGlobal("fetch", fetchMock);
    try {
      await generateMealFact({ trimester: "t2", avoidFacts: ["Folat penting banget di trimester 2.", "Kalsium jaga tulang bayi."] });
      const contextText = JSON.parse(fetchMock.mock.calls[0][1].body).contents[0].parts[0].text;
      expect(contextText).toContain("SUDAH dipakai");
      expect(contextText).toContain("Folat penting banget di trimester 2.");
      expect(contextText).toContain("Kalsium jaga tulang bayi.");
      expect(contextText).toContain("JANGAN ulangi");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_MODEL;
    }
...
// SYSTEM_PROMPT itself isn't exported (it's a static string, not a function to
// unit-test in isolation) -- these assert on what actually reaches Gemini via
// streamNutritionChatTurn's request body, same "observe the request, not the
// private constant" approach as the response-schema-shape tests above. Each
// assertion pins down one specific wording fix from the "make the chat more
// flexible" improvement, so a future edit that accidentally narrows the
// instructions back down fails loudly here instead of silently regressing.
describe("SYSTEM_PROMPT content (via streamNutritionChatTurn's request body)", () => {
  ...
  it("instructs exhaustive extra_nutrients extraction (not just the micronutrient examples) for both food and vitamin categories", async () => {
    const prompt = await getSystemPromptText();
    // Common macros that used to fall through the cracks (see the canned-milk
    // repro in docs/improver/gemini-chat-flexible-assessment.md) are now named
    // explicitly, not left implicit behind "Omega-3, Zinc, Vitamin B6, dll".
    expect(prompt).toContain("Karbohidrat");
    expect(prompt).toContain("Laktosa");
    // The "don't cherry-pick" instruction, present once per category branch.
    expect(prompt.match(/jangan pilih-pilih/g)).toHaveLength(2);
    // The explicit "never rationalize a missing value" rule.
    expect(prompt).toContain("JANGAN PERNAH bilang itu di luar cakupan/tidak didukung Bloom");
  });

  it("allows any topic in category=\"chat\", not just pregnancy/nutrition", async () => {
    const prompt = await getSystemPromptText();
    expect(prompt).toContain("APA PUN yang bukan cerita/foto soal makanan/minuman/vitamin/suplemen");
    expect(prompt).toContain("JANGAN menolak atau");
    expect(prompt).toContain("mengalihkan pembicaraan hanya karena topiknya di luar kehamilan/gizi");
  });
});

// Per-user nutrient goal direction (lib/nutrition.js's resolveEffectiveGoals)
// — a user can flip any nutrient's min/max direction, so the prompt's
// "target minimum" vs "batas maksimum harian" field lists (and the VERDICT
// section's prose naming which nutrients are limits) must follow THIS
// user's effectiveGoals, not the fixed NUTRIENT_ORDER/LIMIT_ORDER split.
describe("effectiveGoals-aware prompt + daily-context hint", () => {
  async function runTurn({ effectiveGoals, dailyLimitTotals, message = "hi" } = {}) {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_CHAT_MODEL = "chat-model-a";
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
    ...
    vi.stubGlobal("fetch", fetchMock);
    try {
      await streamNutritionChatTurn({ message, image: null, history: [], dailyLimitTotals, effectiveGoals }, () => {});
      const requestBody = JSON.parse(fetchMock.mock.calls[0][1].body);
      return {
        systemPrompt: requestBody.systemInstruction.parts[0].text,
        lastTurnText: requestBody.contents.at(-1).parts[0].text,
      };
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_CHAT_MODEL;
  ...
  it("moves a flipped-to-max fixed nutrient out of the 'target minimum' list and into 'batas maksimum harian'", async () => {
    // Built via the real resolveEffectiveGoals (not a hand-rolled partial
    // object) so every ALL_TRACKED_NUTRIENTS key is present, same as the
    // one real caller (app/api/nutrition-chat/route.js) always provides --
    // only protein_g's direction is overridden here.
    const effectiveGoals = resolveEffectiveGoals(
      [{ nutrient_key: "protein_g", label: "Protein", unit: "g", goal_type: "max", target_value: 60 }],
      "t2"
    );
    const { systemPrompt } = await runTurn({ effectiveGoals });
    // The whole prompt is built from ALL_TRACKED_NUTRIENTS + effectiveGoals
    // (missing keys default to goalType !== "max" i.e. min, per
    // buildMinMaxListsForPrompt), so protein_g (flipped here) must appear in
    // the max-type clause, not the min-type one.
    const minClauseMatch = systemPrompt.match(/target minimum" berikut[^:]*:\s*([^.]*)\./);
    const maxClauseMatch = systemPrompt.match(/batas maksimum harian" berikut[^:]*:\s*([\s\S]*?)\. Kalau fotonya/);
    expect(maxClauseMatch[1]).toContain('protein_g (g, "Protein")');
    expect(minClauseMatch[1]).not.toContain('protein_g (g, "Protein")');
  });

  it("uses the user's own target value (not the hardcoded default) in the VERDICT daily-context hint", async () => {
    // sodium_mg customized lower (1500) than the app's own default (2300)
    const effectiveGoals = resolveEffectiveGoals(
      [{ nutrient_key: "sodium_mg", label: "Natrium", unit: "mg", goal_type: "max", target_value: 1500 }],
      "t2"
    );
    const { lastTurnText } = await runTurn({
      effectiveGoals,
      dailyLimitTotals: { sodium_mg: 900 },
      message: "kopi hitam",
    });
    expect(lastTurnText).toContain("Natrium 900mg dari batas 1500mg");
    expect(lastTurnText).toContain("kopi hitam");
  });

  it("excludes a custom (non-fixed) nutrient goal from the prompt's fixed min/max lists", async () => {
    // nutrient_key ("laktosa_intoleransi") is the discriminating check, not
    // the label ("Laktosa") -- that word already appears generically as an
    // example in the exhaustive-extraction instructions regardless of any
    // configured goal, so it can't tell "included" from "not included".
    const effectiveGoals = resolveEffectiveGoals(
      [{ nutrient_key: "laktosa_intoleransi", label: "Laktosa", unit: "g", goal_type: "max", target_value: 5 }],
      "t2"
    );
    // Custom goals are deliberately NOT wired into Gemini's verdict reasoning
    // (see the improvement plan's Out of scope section) -- only the 15
    // fixed ALL_TRACKED_NUTRIENTS keys ever appear in these prompt clauses,
    // regardless of what custom goals a user has configured.
    const { systemPrompt } = await runTurn({ effectiveGoals });
    expect(systemPrompt).not.toContain("laktosa_intoleransi");
    // The fixed max-type defaults are still there, untouched by the unrelated custom entry.
    expect(systemPrompt).toContain('sodium_mg (mg, "Natrium")');
  });

  it("falls back to today's fixed NUTRIENT_ORDER/LIMIT_ORDER split when effectiveGoals is omitted", async () => {
    const { systemPrompt } = await runTurn({});
    // Labels come straight from LIMIT_META (Title Case, e.g. "Gula"/"Natrium")
    // same as every other per-user-aware render of these labels elsewhere in
    // this codebase (Dashboard, etc.) -- not the original hand-written
    // lowercase prose, which only existed because this clause used to be a
    // literal hardcoded sentence rather than derived from label metadata.
    expect(systemPrompt).toContain("asupan Gula/Natrium/Kolesterol/Lemak jenuh/Kafein yang SUDAH");
    expect(systemPrompt).toContain('sodium_mg (mg, "Natrium")');
  });
});
...
// GEMINI_CHAT_API_KEY: an optional, tier-specific key chain for the chat
// tier only (streamNutritionChatTurn/streamTranscribeVoiceNote) -- separate
// from GEMINI_API_KEY, which the low tier (cron: generateDailyNudge/
// generateMealFact) always uses regardless. Entirely opt-in: unset, nothing
// changes from the untiered behavior above (see the first test here, which
// doubles as the backward-compat guard).
describe("GEMINI_CHAT_API_KEY (tier-specific chat key, opt-in)", () => {
  it("falls back to GEMINI_API_KEY for the chat tier when GEMINI_CHAT_API_KEY isn't set", async () => {
    ...
    try {
      await streamNutritionChatTurn({ message: "hi", image: null, history: [] }, () => {});
      expect(fetchMock.mock.calls[0][0]).toContain("key=shared-key");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_CHAT_MODEL;
    }
  ...
  it("uses GEMINI_CHAT_API_KEY for the chat tier when it's set, not GEMINI_API_KEY", async () => {
    process.env.GEMINI_API_KEY = "utility-key";
    process.env.GEMINI_CHAT_API_KEY = "chat-only-key";
    process.env.GEMINI_CHAT_MODEL = "chat-model-a";
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      body: fakeSseBody(JSON.stringify({ category: "chat", reply: "Halo!" })),
    ...
    try {
      await streamNutritionChatTurn({ message: "hi", image: null, history: [] }, () => {});
      expect(fetchMock.mock.calls[0][0]).toContain("key=chat-only-key");
      expect(fetchMock.mock.calls[0][0]).not.toContain("key=utility-key");
    } finally {
      vi.unstubAllGlobals();
  ...
  it("never lets the low tier (cron) pick up GEMINI_CHAT_API_KEY, even when it's set", async () => {
    process.env.GEMINI_API_KEY = "utility-key";
    process.env.GEMINI_CHAT_API_KEY = "chat-only-key";
    process.env.GEMINI_MODEL = "low-model-a";
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      json: async () => ({ candidates: [{ content: { parts: [{ text: JSON.stringify({ nudge: "Semangat!" }) }] } }] }),
    });
    vi.stubGlobal("fetch", fetchMock);
    try {
      await generateDailyNudge({});
      expect(fetchMock.mock.calls[0][0]).toContain("key=utility-key");
      expect(fetchMock.mock.calls[0][0]).not.toContain("key=chat-only-key");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_CHAT_API_KEY;
      delete process.env.GEMINI_MODEL;
  ...
  it("uses GEMINI_API_KEY (not the chat-only key) on the last-resort drop from chat tier to low tier", async () => {
    process.env.GEMINI_API_KEY = "utility-key";
    process.env.GEMINI_CHAT_API_KEY = "chat-only-key"; // exhausted below -- must not leak into the low-tier fallback attempt
    process.env.GEMINI_CHAT_MODEL = "chat-model-a";
    process.env.GEMINI_MODEL = "low-model-a";
    const fetchMock = vi.fn(async (url) => (
      url.includes("low-model-a")
        ? { ok: true, body: fakeSseBody(JSON.stringify({ category: "chat", reply: "Halo dari model cadangan!" })) }
        : { ok: false, status: 429, text: async () => "quota exceeded" } // chat-model-a fails
    ));
    vi.stubGlobal("fetch", fetchMock);
    try {
      await streamNutritionChatTurn({ message: "hi", image: null, history: [] }, () => {});
      expect(fetchMock).toHaveBeenCalledTimes(2); // chat-model-a (fails), then low-model-a (succeeds)
      expect(fetchMock.mock.calls[0][0]).toContain("key=chat-only-key");
      expect(fetchMock.mock.calls[1][0]).toContain("key=utility-key");
      expect(fetchMock.mock.calls[1][0]).not.toContain("key=chat-only-key");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_CHAT_API_KEY;
      delete process.env.GEMINI_CHAT_MODEL;
