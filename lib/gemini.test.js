...
// model-fallback-on-429 chain specifically — distinct from the retry-per-model
// policy above, this is "move to a *different* model" rather than "retry the
// same one". No-fallback-configured behavior is already covered by the two
// tests above (neither sets GEMINI_FALLBACK_MODELS), so it's not repeated here.
describe("low-tier model fallback chain (GEMINI_MODEL / GEMINI_FALLBACK_MODELS, via generateDailyNudge)", () => {
  ...
  it("does not fall back to the next low-tier model on a non-429 failure", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_MODEL = "low-model-a";
    process.env.GEMINI_FALLBACK_MODELS = "low-model-b";
    const fetchMock = vi.fn().mockResolvedValue({ ok: false, status: 503, text: async () => "high demand" });
    vi.stubGlobal("fetch", fetchMock);
    try {
      await expect(generateDailyNudge({})).rejects.toMatchObject({ status: 503 });
      // low-model-a's own retry policy (503 is retryable) accounts for all 3 calls — low-model-b is never touched
      expect(fetchMock).toHaveBeenCalledTimes(3);
      for (const [url] of fetchMock.mock.calls) expect(url).toContain("low-model-a");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_MODEL;
      delete process.env.GEMINI_FALLBACK_MODELS;
  ...
  it("exhausts the whole low-tier chain when every model 429s", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_MODEL = "low-model-a";
    ...
    try {
      await expect(generateDailyNudge({})).rejects.toMatchObject({ status: 429 });
      expect(fetchMock).toHaveBeenCalledTimes(3); // one per model in the chain, no retries (429 each)
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_MODEL;
      delete process.env.GEMINI_FALLBACK_MODELS;
...
describe("chat tier + cross-tier fallback (streamNutritionChatTurn, streamTranscribeVoiceNote)", () => {
  ...
  it("does not fall through to the low tier on a non-429 chat-tier failure", async () => {
    process.env.GEMINI_API_KEY = "test-key";
    process.env.GEMINI_CHAT_MODEL = "chat-model-a";
    process.env.GEMINI_MODEL = "low-model-a";
    ...
    try {
      ...
      // chat-model-a's own retry policy (500 is retryable) accounts for all 3 calls — low-model-a is never touched
      expect(fetchMock).toHaveBeenCalledTimes(3);
      for (const [url] of fetchMock.mock.calls) expect(url).toContain("chat-model-a");
    } finally {
      vi.unstubAllGlobals();
      delete process.env.GEMINI_CHAT_MODEL;
      delete process.env.GEMINI_MODEL;
