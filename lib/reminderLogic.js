...
// missingMeal/missingVitamin/limitWarning is truthy (the route skips sending
// entirely otherwise) — passed none of them, it still returns a sane (if
// unused) string rather than throwing, so a caller mistake fails loud in a
// test instead of at 19:00 in production. `name` is optional (see greet()
// above). `limitWarning` (optional): buildLimitWarningClause's output below —
// appended after the nudge in EVERY lead branch, including "Semua sudah
// tercatat hari ini!" (exactly the case this unlocks: fully logged, but over
// a daily limit). Space-joined, not newline-joined — push notification
// bodies don't reliably render multi-line text.
export function buildReminderBody({ missingMeal, missingVitamin, nudge, name, limitWarning }) {
  let lead;
  ...
  const base = nudge ? `${prefixed} ${nudge}` : prefixed;
  return limitWarning ? `${base} ${limitWarning}` : base;
}

// Deterministic, exact-math warning clause for the dinner slot — shared
// source of truth with the Dashboard's own warning banner
// (app/dashboard/page.js), both driven by lib/nutrition.js's
// exceededLimitLabels so "what counts as exceeded" can't drift between the
// two. Returns "" for an empty list (buildReminderBody then adds nothing).
export function buildLimitWarningClause(exceededLabels) {
  if (!exceededLabels || exceededLabels.length === 0) return "";
  return `⚠️ Sudah melebihi batas harian ${exceededLabels.join(", ")} hari ini — coba dikurangi dulu ya.`;
}

// Fixed, deterministic line for the morning slot's weekly weigh-in check
// (app/api/cron/reminders/route.js decides WHETHER to pass this — no
// weight_logs entry in the last 7 days) — a static check like this doesn't
// need a Gemini call the way the affirmation/fact `nudge` does.
export const WEIGHT_NUDGE_TEXT = "📏 Belum catat berat badan 7 hari terakhir — yuk timbang & catat di Dashboard.";
...
// reused for morning/dinner/night), plus an optional weekly weight-nudge
// clause (space-appended, same pattern as buildReminderBody's limitWarning).
export function buildMorningBody({ name, nudge, weightNudge }) {
  const base = `${greet(name, "Pagi")} ${nudge}`.trim();
  return weightNudge ? `${base} ${weightNudge}` : base;
}
