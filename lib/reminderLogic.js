...
// Sums each user's { sent, pruned, skipped } result (one per settled task in
// app/api/cron/reminders/route.js's Promise.all over userIds) into the
// route's final response totals. Pulled out as its own pure function -- like
// everything else in this file -- so the one bit of the route's own logic
// that *can* be unit-tested without mocking Gemini/Supabase/web-push
// actually is, instead of only being exercised by production traffic.
