...
// WHO adult BMI categories — same boundary convention as lib/nutrition.js's
// STATUS_TIERS (a boundary value belongs to the *higher* category): 18.5
// itself is "normal" (not "underweight"), 25 is "overweight" (not "normal"),
// 30 is "obese".
//
// IMPORTANT: these are exactly the thresholds IOM's 2009 pregnancy gain
// guidance keys off of, but ONLY when applied to PRE-PREGNANCY weight — never
// a current/already-gained weight. A pregnant woman's weight rises through
// the pregnancy itself, so feeding a later weight_logs value in here would
// silently reclassify her into a higher BMI category as she gains exactly as
// expected, which is backwards. Every caller in this app MUST compute the BMI
// from profiles.pre_pregnancy_weight_kg (see app/dashboard/profile/page.js),
// never from a weight_logs row.
...
// Groups weight_logs rows (`{date, weight_kg}`, any order) by calendar month
// ("YYYY-MM"), keeping only each month's LAST entry (by date) as that
// month's representative weight, then chains month-over-month deltas:
// month M's gain = (last weight in M) − baseline, where baseline is the
// last weight in the most recent PRIOR month that has data (not necessarily
// the literal previous calendar month — a data gap just chains to whichever
// month came before it), or `prePregWeightKg` for the very first data month
// (if set). A month with no usable baseline (no prior data month AND no
// pre-pregnancy weight) is simply OMITTED from the result, not shown as a
// misleading 0kg bar — used by the "Peningkatan berat bulanan" monthly bar
// chart (app/dashboard/profile/page.js).
export function computeMonthlyGain(weightRows, prePregWeightKg) {
  const lastOfMonth = {};
  (weightRows || []).forEach((r) => {
    if (!r?.date) return;
    const month = r.date.slice(0, 7);
    if (!lastOfMonth[month] || r.date > lastOfMonth[month].date) lastOfMonth[month] = r;
  });
  const months = Object.keys(lastOfMonth).sort();
  const result = [];
  const basePrePreg = Number(prePregWeightKg);
  let prevWeight = Number.isFinite(basePrePreg) && basePrePreg > 0 ? basePrePreg : null;
  months.forEach((month) => {
    const weight = Number(lastOfMonth[month].weight_kg);
    if (prevWeight != null && Number.isFinite(weight)) {
      result.push({ month, gainKg: weight - prevWeight });
    }
    prevWeight = Number.isFinite(weight) ? weight : prevWeight;
  });
  return result;
}
